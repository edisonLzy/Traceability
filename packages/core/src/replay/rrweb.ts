...
function buildReplayBody(replayId: string, sentryEventId?: string): RrwebReplayIngestBody | undefined {
...
function pruneEvents(): void {
  ...
  const fullSnapshotAfterOverflow = events.findIndex((event, index) => (
    index >= overflow && event.type === EventType.FullSnapshot
  ))
