...
export function installPerformanceMonitoring(report: ReportMetric): void {
  ...
  const emitNavigation = () => {
    ...
    emit({ name: 'TTFB', value: Math.max(0, navigation.responseStart - navigation.startTime), unit: 'millisecond' })
    emit({ name: 'DOMContentLoaded', value: Math.max(0, navigation.domContentLoadedEventEnd - navigation.startTime), unit: 'millisecond' })
  }
...
function observe(type: string, callback: (entries: EntryWithFields[]) => void, durationThreshold?: number): void {
  if (typeof PerformanceObserver === 'undefined') return
  try {
    const observer = new PerformanceObserver((list) => callback(list.getEntries() as EntryWithFields[]))
