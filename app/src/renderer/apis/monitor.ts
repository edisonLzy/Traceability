...
export async function getIssueEvents(issueId: string): Promise<Event[]> {
  const { data } = await request.get<Event[]>(
    `/api/issues/${requiredId(issueId, "issueId")}/events`,
  );
  return data;
}
...
export async function getIssueReplays(issueId: string): Promise<RrwebReplaySummary[]> {
  const { data } = await request.get<RrwebReplaySummary[]>(
    `/api/issues/${requiredId(issueId, "issueId")}/replays`,
  );
  return data;
}
