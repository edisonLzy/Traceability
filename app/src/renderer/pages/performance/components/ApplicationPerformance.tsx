      ...
      <CardHeader className="min-h-0 items-start px-4 py-3">
        <div>
          ...
          <div className="mt-0.5 text-xs text-tertiary">
            {app.appId.slice(0, 8)} · {app.samples} samples
          </div>
        </div>
      </CardHeader>
      <Table>
        ...
        <TableBody>
          {Object.entries(app.metrics)
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([name, metric]) => (
              <TableRow key={name}>
                <TableCell>
                  <span className="font-mono text-xs text-[#aeb7ff]">{name}</span>
                </TableCell>
                ...
                <TableCell className="text-subtle">
                  {new Date(metric.lastSeen).toLocaleString()}
                </TableCell>
                <TableCell>
                  <Button
                    size="sm"
          ...
          {Object.keys(app.metrics).length === 0 && (
            <TableRow>
              <TableCell colSpan={6} className="text-subtle">
                No performance samples received yet.
              </TableCell>
            </TableRow>
          )}
        </TableBody>
