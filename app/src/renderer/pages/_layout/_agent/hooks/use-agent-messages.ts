...
async function persistUnsyncedEntries(sessionId: string): Promise<void> {
  const store = agentStore.getState();
  const entries = store
    .getEntryState(sessionId)
  ...
  useSubscribeAgentEvents(
    {
      ...
      agent_end: async (event) => {
        ...
        if (!persistedRunRef.current[event.sessionId]) {
          ...
          await persistUnsyncedEntries(event.sessionId);
        }
      },
      ...
      message_start: (event) => {
        if (isUserMessage(event.message)) {
          if (event.message.kind !== "steering") {
      ...
      ask_user_question_requested: (event) => {
        ...
        agentStore.getState().enqueueQuestion(sessionId, question as AskUserQuestionRequest);
      },
    },
