    ...
    <div className="mx-auto block min-h-full max-w-[1440px] px-4 pt-5.5 pb-15 tablet:px-8 tablet:pt-7">
      ...
      {apps.length === 0 ? (
        <div className="px-5 py-13.5 text-center text-subtle">
          No applications yet. Create one to get a DSN.
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3 tablet:grid-cols-2 desktop:grid-cols-3">
          {apps.map((a) => (
            <article
              key={a.id}
              className="cursor-pointer rounded-xl border border-hairline bg-surface-1 p-4.5 transition-colors hover:border-hairline-strong hover:bg-surface-2"
              onClick={() => nav(`/apps/${a.id}`)}
            >
              <div className="flex items-start gap-3">
                <div className="grid size-9.5 place-items-center rounded-lg bg-[#202128] font-semibold text-muted">
                  {a.name.slice(0, 2).toUpperCase()}
                </div>
                <div className="min-w-0">
                  <div className="font-medium">{a.name}</div>
                  <div className="overflow-hidden text-ellipsis whitespace-nowrap text-[11px] text-tertiary">
                    {a.repoUrl}
                  </div>
                </div>
              ...
              <div className="mt-5 flex gap-6">
                <div>
                  <b className="block text-lg font-semibold">{a.defaultBranch}</b>
                  <span className="text-[11px] text-tertiary">Default branch</span>
                </div>
                <div>
                  <b className="block text-lg font-semibold">{a.id.slice(0, 6)}</b>
                  <span className="text-[11px] text-tertiary">App ID</span>
                </div>
              </div>
            </article>
          ))}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Connect a repository and generate its appId and DSN.
            </DialogDescription>
          </DialogHeader>
          <DialogBody>
            <Field
