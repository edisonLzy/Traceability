  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        {appId && <button className="btn btn-primary" onClick={() => {
  ...
  return (
    <section className="panel performance-card">
      ...
      <table className="data-table">
        <thead><tr><th>Metric</th><th>Average</th><th>p75</th><th>Samples</th><th>Last seen</th><th></th></tr></thead>
        <tbody>
          {Object.entries(app.metrics).sort(([a], [b]) => a.localeCompare(b)).map(([name, metric]) => (
            <tr key={name}>
              ...
              <td><button className="btn btn-sm" onClick={() => window.dispatchEvent(new CustomEvent('traceability:agent-context', {
                detail: { appId: app.appId, source: 'metric', metricName: name, hours },
              }))}>Ask Agent</button></td>
            </tr>
          ))}
          {Object.keys(app.metrics).length === 0 && <tr><td colSpan={6} className="muted">No performance samples received yet.</td></tr>}
        </tbody>
      </table>
    </section>
  )
