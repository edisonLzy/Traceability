...
describe("AgentRuntime", () => {
  ...
  it("persists a normal response and resolves an Issue tool call through the self-contained monitor client", async () => {
    ...
    try {
      ...
      const runtime = new AgentRuntime(created.id, created.appId, sessions, models, http, (event) =>
      ...
      expect(detail?.entries.slice(1).map((entry) => entry.data.role)).toEqual([
        "user",
        "assistant",
        "toolResult",
        "assistant",
      ]);
      expect(JSON.stringify(detail?.entries)).toContain(
        "The issue was loaded through the monitoring tool.",
      );
  ...
  it("passes Performance summaries through a monitor tool call before the assistant replies", async () => {
    ...
    try {
      ...
      expect(http.get).toHaveBeenCalledWith(
        expect.stringContaining("/api/performance?appId=app-performance&hours=24"),
      );
      expect(JSON.stringify(sessions.get(created.id)?.entries)).toContain(
        "Performance data shows the requested monitoring summary.",
      );
    } finally {
      faux.unregister();
    }
