...
export class SessionStore {
  ...
  recoverInterruptedRuns(): void {
    ...
    this.db.transaction(() => {
      const sessionIds = (
        this.db.raw
          .prepare("SELECT DISTINCT session_id FROM agent_runs WHERE status = 'running'")
      ...
      if (sessionIds.length > 0) {
        const update = this.db.raw.prepare(
          "UPDATE agent_sessions SET status = 'interrupted', updated_at = ? WHERE id = ?",
        );
  ...
  get(sessionId: string): AgentSessionDetail | null {
    const row = this.db.raw.prepare("SELECT * FROM agent_sessions WHERE id = ?").get(sessionId) as
      | SessionRow
      | undefined;
    if (!row) return null;
    return {
  ...
  updateRunSnapshot(runId: string, message: Record<string, unknown>): void {
    this.db.raw
      .prepare("UPDATE agent_runs SET partial_message_json = ? WHERE id = ? AND status = 'running'")
  ...
  completeRun(
    ...
    const run = this.db.raw.prepare("SELECT * FROM agent_runs WHERE id = ?").get(runId) as
      | RunRow
      | undefined;
    if (!run) return;
    const now = Date.now();
    const sessionStatus: AgentSessionStatus =
      status === "failed" ? "failed" : status === "interrupted" ? "interrupted" : "idle";
    this.db.transaction(() => {
      this.db.raw
        .prepare(`
        UPDATE agent_runs
  ...
  private appendEntry(
    ...
    this.db.raw
      .prepare("UPDATE agent_sessions SET title = ?, updated_at = ? WHERE id = ?")
      .run(title ?? session.title, now, sessionId);
    return toEntry(row);
  }
  ...
  private getArtifacts(sessionId: string): AgentArtifact[] {
    const rows = this.db.raw
      .prepare("SELECT * FROM agent_artifacts WHERE session_id = ? ORDER BY updated_at DESC")
  ...
  private getLatestRun(sessionId: string): AgentRun | null {
    const row = this.db.raw
      .prepare("SELECT * FROM agent_runs WHERE session_id = ? ORDER BY started_at DESC LIMIT 1")
