...
export class MonitorClient {
  ...
  async getIssue(issueId: string): Promise<unknown> {
    const result = await this.http.get(`/api/issues/${requiredId(issueId, 'issueId')}`).then((r) => r.data)
  ...
  async getIssueEvents(issueId: string): Promise<unknown> {
    ...
    const result = await this.http.get(`/api/issues/${requiredId(issueId, 'issueId')}/events`).then((r) => r.data)
    return monitorResultSchemas.getIssueEvents.parse(result)
  }

  async getIssueReplays(issueId: string): Promise<unknown> {
    await this.assertIssueScope(issueId)
    const result = await this.http.get(`/api/issues/${requiredId(issueId, 'issueId')}/replays`).then((r) => r.data)
  ...
  async getReplay(issueId: string, replayId: string): Promise<unknown> {
    ...
    const result = await this.http.get(`/api/issues/${issue}/replays/${replay}`).then((r) => r.data)
  ...
  function createTool(
    ...
    return {
      ...
      async execute(_toolCallId, args) {
        ...
        const result = method === 'listIssues' ? await client.listIssues({ status: asString(a.status), limit: asNumber(a.limit) })
          : method === 'getIssue' ? await client.getIssue(requiredString(a.issueId, 'issueId'))
          : method === 'getIssueEvents' ? await client.getIssueEvents(requiredString(a.issueId, 'issueId'))
          : method === 'getIssueReplays' ? await client.getIssueReplays(requiredString(a.issueId, 'issueId'))
          : method === 'getReplay' ? await client.getReplay(requiredString(a.issueId, 'issueId'), requiredString(a.replayId, 'replayId'))
