...
if (!appId) {
  throw new Error('TRACEABILITY_DEMO_APP_ID is required. Create an application first, then export its id before uploading source maps.')
}
...
for (const mapPath of maps) {
  ...
  const file = relative(dist, mapPath).replace(/\\/g, '/').replace(/\.map$/, '')
  const response = await fetch(`${server}/api/apps/${appId}/sourcemaps`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ release, file, sourceMap }),
  })
  if (!response.ok) throw new Error(`Could not upload ${file}: ${response.status} ${await response.text()}`)
