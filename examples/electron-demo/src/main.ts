...
monitor.handle("demo:ipc-echo", (_event, payload) => ({
  ok: true,
  received: payload,
...
monitor.handle("demo:main-uncaught-exception", () => {
  setTimeout(() => {
    throw new Error("demo: Electron main-process uncaught exception");
  }, 0);
  ...
  const window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: fileURLToPath(
        new URL("../../../packages/electron/dist/preload.cjs", import.meta.url),
      ),
    },
  });
...
app.whenReady().then(async () => {
  ...
  if (process.env.ELECTRON_DEMO_CRASH_ON_START === "1") {
    setTimeout(() => {
      throw new Error("demo: Electron main-process startup crash");
    }, 300);
  }
});
