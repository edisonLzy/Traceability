  ...
  return {
    ...
    summary(opts: { appId?: string; hours?: number }): PerformanceSummary {
      ...
      const applicationRows = db.prepare(
        opts.appId ? 'SELECT id, name FROM applications WHERE id = ?' : 'SELECT id, name FROM applications',
      ).all(...(opts.appId ? [opts.appId] : [])) as ApplicationRow[]
      for (const application of applicationRows) {
        if (!apps.has(application.id)) {
          apps.set(application.id, { appId: application.id, appName: application.name, samples: 0, values: new Map() })
        }
      }
